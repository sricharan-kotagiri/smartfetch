...
router.post('/', authMiddleware, asyncHandler(async (req, res) => {
    ...
    const { shop_id, items, payment_method, pickup_time } = req.body;
    if (!shop_id || !items || items.length === 0 || !payment_method || !pickup_time) {
        const error = new Error('shop_id, items, payment_method, and pickup_time are required');
    ...
    try {
        ...
        // Build QR data
        const qrData = buildQRData('', // Will be set after order creation
        pickupCode, customer.full_name, shop.shop_name, shopkeeper.full_name, orderItems.map(item => ({
            name: item.product_name,
            qty: item.quantity,
            unitPrice: item.unit_price,
            subtotal: item.subtotal
        })), totalAmount, payment_method, pickup_time, new Date().toISOString());
        // Create order
        ...
        if (orderError) {
            const apiError = new Error(orderError.message);
            apiError.statusCode = 400;
            throw apiError;
        }
        const orderId = orderData?.[0]?.id || orderData?.id;
...
router.get('/customer', authMiddleware, asyncHandler(async (req, res) => {
    ...
    try {
        ...
        const apiError = new Error(error.message || 'Failed to fetch orders');
        apiError.statusCode = 400;
        throw apiError;
    }
}));
 ...
 */
router.get('/shop/:shop_id', authMiddleware, asyncHandler(async (req, res) => {
    const userId = req.user?.id;
    ...
    try {
        ...
        if (!shop || shop.shopkeeper_id !== userId) {
            const error = new Error('Unauthorized to view these orders');
...
router.get('/:id', authMiddleware, asyncHandler(async (req, res) => {
    ...
    try {
        ...
        const apiError = new Error(error.message || 'Failed to fetch order');
        apiError.statusCode = error.statusCode || 400;
        throw apiError;
    }
}));
...
router.patch('/:id/status', authMiddleware, asyncHandler(async (req, res) => {
    ...
    try {
        ...
        const apiError = new Error(error.message || 'Failed to update order status');
        apiError.statusCode = error.statusCode || 400;
        throw apiError;
    }
}));
 ...
 * Get order receipt
 */
router.get('/:id/receipt', authMiddleware, asyncHandler(async (req, res) => {
    const { id } = req.params;
    try {
        const { data: order, error } = await supabase
            .from('orders')
        ...
        const apiError = new Error(error.message || 'Failed to fetch receipt');
        apiError.statusCode = error.statusCode || 400;
        throw apiError;
    }
}));
